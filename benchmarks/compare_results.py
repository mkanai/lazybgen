#!/usr/bin/env python3
"""
Compare benchmark results from multiple lazybgen commits.
...
class BenchmarkComparator:
    ...
    def extract_metrics(self):
        ...
        lazybgen runs several reader workloads per file (load_full, load_region,
        load_sample_filtered, iter_variants). Each (config, workload) pair is a
        distinct comparison row, keyed as "<config>::<workload>", so the per-row
        machinery below (and the regression detector) works unchanged.
        """
        for commit, data in self.results.items():
            commit_info = data.get("commit_info", commit)

            for benchmark in data["benchmarks"]:
                config_name = benchmark["config"]

                for workflow_name, workflow_data in benchmark.get("workflows", {}).items():
                    agg = workflow_data.get("aggregate", {})
                    if agg.get("success_rate", 0) <= 0:
                        continue

                    derived = workflow_data.get("derived", {})
                    row = {
                        "commit": commit,
                        "commit_info": commit_info,
                        "timestamp": data["timestamp"],
                        "config": f"{config_name}::{workflow_name}",
                        "workflow": workflow_name,
                        "samples": benchmark["samples"],
                        "variants": benchmark["variants"],
                        "compression": benchmark["compression"],
                        "file_size_mb": benchmark["file_size_mb"],
                        "median_time": agg.get("median_time", np.nan),
                        "mean_time": agg.get("mean_time", np.nan),
                        "std_time": agg.get("std_time", np.nan),
                        "min_time": agg.get("min_time", np.nan),
                        "max_time": agg.get("max_time", np.nan),
                        "median_memory_mb": agg.get("median_memory_mb", np.nan),
                        "variants_per_second": derived.get("variants_per_second", np.nan),
                        "mb_per_second": derived.get("mb_per_second", np.nan),
                    }
                        ...
                        if not np.isnan(baseline_value) and baseline_value != 0:
                            if metric in ["variants_per_second", "mb_per_second"]:
                                ...
                                self.df.loc[config_mask, f"{metric}_pct_change"] = (
                                    (self.df.loc[config_mask, metric] - baseline_value) / baseline_value
                                ) * 100
                            else:
                                # Lower is better
                                self.df.loc[config_mask, f"{metric}_pct_change"] = (
        ...
        with open(summary_file, "w") as f:
            ...
            for config in sorted(self.df["config"].unique()):
                ...
                for commit in sorted_commits:
                    ...
                    f.write(
                        f"| {commit} | {time_val:.2f} | {time_change} | "
            ...
            if regressions:
                ...
                for reg in sorted(regressions, key=lambda x: x["time_increase"], reverse=True):
                    f.write(
                        f"- **{reg['config']}**: {reg['time_increase']:.1f}% slower "
    ...
    def export_json(self):
        ...
        for config in self.df["config"].unique():
            ...
            comparison_json["summary"][config] = {
                ...
                "median_time_range": [float(config_data["median_time"].min()), float(config_data["median_time"].max())],
        ...
        if "variants_per_second" in self.df.columns:
            ...
            for config in sorted(self.df["config"].unique()):
                ...
                if len(config_data) > 1 and not config_data["variants_per_second"].isna().all():
                    ax.plot(
                        config_data["commit"],
                        config_data["variants_per_second"],
                        marker="o",
                        label=config,
                        linewidth=2,
                        markersize=8,
...
def main():
    parser = argparse.ArgumentParser(description="Compare lazybgen benchmark results")
    parser.add_argument("results", nargs="+", help="Benchmark result JSON files")
    parser.add_argument(
        "-o", "--output-dir", default="comparison_results", help="Output directory for comparison results"
    )
    parser.add_argument(
        "--baseline",
        default=None,
    ...
    # Validate input files. Accept already-resolved paths (shell glob expansion)
    # as well as glob patterns relative to the CWD or an absolute directory.
    result_files = []
