...
def create_bgen_file(output_path, n_samples, n_variants, compression="zlib", bit_depth=8, use_zstd=False, layout=2):
    """
    ...
    writer = BgenWriter(str(output_path), n_samples=n_samples, compression=compression, layout=layout, samples=samples)
    ...
    for i in range(n_variants):
        ...
        writer.add_variant(varid, rsid, chrom, pos, alleles, genotypes, phased=False, bit_depth=bit_depth)
...
def create_bgi_index(bgen_path):
    ...
    try:
        subprocess.run([str(bgenix_path), "-g", str(bgen_path), "-index"], capture_output=True, text=True, check=True)
...
def get_test_configurations(mode="standard"):
    ...
    # UKBB-shaped: many samples, few variants (mirrors run_benchmark.py 'large').
    large_configs = [
        {"name": "ukbb_200k", "n_samples": 200000, "n_variants": 500, "compression": "zlib", "bit_depth": 8},
        {"name": "ukbb_500k", "n_samples": 500000, "n_variants": 300, "compression": "zlib", "bit_depth": 8},
        {"name": "ukbb_500k_zstd", "n_samples": 500000, "n_variants": 300, "use_zstd": True, "bit_depth": 8},
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=None, help="Output directory (default: benchmarks/test_data)"
    )
