        ...
        with open(stats_file, "r") as f:
            ...
            for i, line in enumerate(lines):
                if "cumulative" in line and i + 2 < len(lines):
                    ...
                    if top_func_line:
                        insights.append({"file": stats_file.name, "top_function": top_func_line})
                    break
...
def main():
    ...
    parser.add_argument("profile_dir", help="Directory containing profile files (e.g., benchmark_results/profiles)")
    parser.add_argument(
        "--output-dir", "-o", help="Output directory for analysis results (default: same as profile_dir)"
    )
    parser.add_argument(
        "--summary-only", action="store_true", help="Only generate summary report without detailed analysis"
    )
